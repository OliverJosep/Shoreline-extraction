...
USE_MLFLOW = True
MLFLOW_EXPERIMENT_NAME = "shoreline_kfold_512x512"

image_type_paths = {
    "oblique": {
        "path": os.path.abspath(
            os.path.join(os.getcwd(), "data/processed_obliques_2_classes/")
        ),
        "num_classes": 2,
    },
    "rectified": {
        "path": os.path.abspath(
            os.path.join(os.getcwd(), "data/processed_rectified_3_classes/")
        ),
        "num_classes": 3,
    },
}
...
patches = {
    "512x512": {"patch_size": (512, 512), "stride": (256, 256)},
}


def generate_patches(data, output_dir, patch_size, stride):
    patchify = Patchify(patch_size, stride)
    patchify.extract_patches_and_save(
        data,
        output_dir,
        skip_no_shoreline=True,
        binary_class=True,
        p_keep_negative=1,
...
def main():
    for data_type in image_type_paths:
        ...
        for fold in data_splits.keys():
            print(f"\n{'=' * 20}\nStarting {fold}\n{'=' * 20}")
            print(
                f"Train size: {len(data_splits[fold]['train']['images'])}, Val size: {len(data_splits[fold]['validation']['images'])}, Test size: {len(data_splits[fold]['test']['images'])}"
            )
            ...
            for key in patches.keys():
                ...
                output_dir = os.path.abspath(
                    os.path.join(
                        os.getcwd(),
                        f"data/{k}fold/{data_type}/{fold}_patchify_{num_classes}_classes_{data_type}_{key}/",
                    )
                )
                if not os.path.exists(output_dir):
                    ...
                    generate_patches(
                        data,
                        output_dir,
                        patches[key]["patch_size"],
                        patches[key]["stride"],
                    )
                    print(f"\tDone generating patches for {key}")
                else:
                    print(
                        f"\tDirectory {output_dir} already exists. Skipping..."
                    )

        output_dir = os.path.abspath(
            os.path.join(os.getcwd(), f"data/{k}fold/{data_type}/")
        )
            ...
            for fold in data_splits.keys():
                ...
                for key in patches.keys():
                    ...
                    model.load_data(
                        ...
                        CNNFormes,
                        batch_size=24,
                        resize_shape=patches[key]["patch_size"],
                        drop_last=True,
                    )
